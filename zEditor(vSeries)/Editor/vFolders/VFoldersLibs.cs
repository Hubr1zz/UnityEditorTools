...
namespace VFolders.Libs
{
    public static class VUtils
    {
        ...
        public static int ToLegacyInstanceId(this int instanceId) => instanceId;
        ...
        public static List<int> ToLegacyInstanceIds(this IEnumerable<int> instanceIds) => instanceIds.ToList();
#endif

        public static List<int> GetLegacyIdList(this object owner, string memberName)
        {
            if (owner == null) return new List<int>();
            object value = owner.GetMemberValue(memberName);
#if UNITY_6000_3_OR_NEWER
            if (value is IEnumerable<EntityId> entityIds) return entityIds.ToLegacyInstanceIds();
#endif
            if (value is IEnumerable<int> instanceIds) return instanceIds.ToList();
            return new List<int>();
        }

        public static Object LegacyInstanceIdToObject(int instanceId)
        {
#if UNITY_6000_3_OR_NEWER
            return EditorUtility.EntityIdToObject(instanceId.ToEntityId());
#else
            return EditorUtility.InstanceIDToObject(instanceId);
#endif
        }

        public static string LegacyInstanceIdToAssetPath(int instanceId)
        {
#if UNITY_6000_3_OR_NEWER
        ...
        public static bool SelectionContainsLegacyId(int instanceId)
        {
#if UNITY_6000_5_OR_NEWER
            return Selection.Contains(instanceId.ToEntityId());
#else
            return Selection.Contains(instanceId);
#endif
        }

        public static int[] SelectedLegacyInstanceIds
        {
            get
            {
#if UNITY_6000_3_OR_NEWER
                return Selection.entityIds.Select(entityId => entityId.ToLegacyInstanceId()).ToArray();
#else
        ...
        public struct GlobalID : System.IEquatable<GlobalID>
        {
            ...
            {
#if UNITY_6000_3_OR_NEWER
                return GlobalObjectId.GlobalObjectIdentifierToEntityIdSlow(globalObjectId).ToLegacyInstanceId();
#else
...
#if UNITY_6000_3_OR_NEWER
            GlobalObjectId.GetGlobalObjectIdsSlow(instanceIds.Select(instanceId => instanceId.ToEntityId()).ToArray(), unityGlobalIds);
#else
        ...
        public static int[] GetObjectInstanceIds(this IEnumerable<GlobalID> globalIDs)
        {
            ...
            for (int i = 0; i < entityIds.Length; i++)
                iids[i] = entityIds[i].ToLegacyInstanceId();
#else
        ...
        public static class EditorUtils
        {
            ...
            public static void OpenFolder(string path)
            {
                ...
                m_ListAreaState.GetType().GetField("m_SelectedInstanceIDs").SetValue(m_ListAreaState, new List<int> { folder.GetLegacyInstanceId() });
