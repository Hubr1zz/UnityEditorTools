    ...
    public static class VFavorites
    {
            ...
            void pages()
            {
                void page(Rect pageRect, Page page)
                {
                    ...
                    void rows()
                    {
                        ...
                        {
                            var rowRect = pageRect.SetHeight(page.rowScale).SetY(y).SetX(0);

                            var iconOffset = 6;
                            var iconSize = 25 * Mathf.Min(1, page.rowScale);
                            var nameOffset = 3;
                            ...
                            void background()
                            {
                                ...
                                var rowColor = Lerp(evenColor, oddColor, rowRect.y.PingPong(page.rowScale) / page.rowScale);
                            ...
                            void crossButton()
                            {
                                ...
                                CancelRowAnimations();
                                data.curPage.rowGaps[data.curPage.items.IndexOf(item)] = page.rowScale;
        ...
        static void UpdateMouseState() // called from WrappedOnGUI 
        {
            ...
            void down()
            {
                ...
                var pressedItemIndex = (mouseDownPosition_rowsSpace.y / data.curPage.rowScale).FloorToInt();
        ...
        static void UpdateAnimations() // Update
        {
            ...
            void rowGaps()
            {
                ...
                for (int i = 0; i < data.curPage.rowGaps.Count; i++)
                    data.curPage.rowGaps[i] = Lerp(data.curPage.rowGaps[i], draggingItem && i == insertDraggedItemAtIndex ? data.curPage.rowScale : 0, lerpSpeed, deltaTime);
            ...
            void droppedItem()
            {
                ...
                SmoothDamp(ref droppedItemY_rowsSpace, data.curPage.items.IndexOf(VFavorites.droppedItem) * data.curPage.rowScale, yLerpSpeed, ref droppedItemYDerivative, deltaTime);
        ...
        static bool animatingRowGaps => data.curPage.rowGaps.Any(r => r > .1f && r < data.curPage.rowScale - .1f);
        ...
        static void UpdateDragging() // called from WrappedOnGUI 
        {
            ...
            void initFromPage()
            {
                ...
                var i = (mouseDownPosition_rowsSpace.y / data.curPage.rowScale).FloorToInt();
                ...
                draggedItemHoldOffset = (i * data.curPage.rowScale + data.curPage.rowScale / 2) - mouseDownPosition_rowsSpace.y;

                data.curPage.lastItemDragTime_ticks = System.DateTime.UtcNow.Ticks;

                data.curPage.items.Remove(draggedItem);
                data.curPage.rowGaps[draggingItemFromPageAtIndex] = data.curPage.rowScale;
        ...
        static void AcceptDragging()
        {
            ...
            data.curPage.rowGaps[insertDraggedItemAtIndex] -= data.curPage.rowScale;
        ...
        static float draggedItemY_groupSpace => (mousePosition_groupSpace.y - data.curPage.rowScale / 2 + draggedItemHoldOffset).Clamp(0, 12321);
        static float draggedItemY_rowsSpace => draggedItemY_groupSpace + data.curPage.scrollPos;
        static int insertDraggedItemAtIndex => ((mousePosition_rowsSpace.y + draggedItemHoldOffset) / data.curPage.rowScale).FloorToInt().Clamp(0, data.curPage.items.Count);
        ...
        static void UpdateGUIWrapping() // called from EditorApplicaton.update 
        {
            void wrap()
            {
                ...
                if (Resources.FindObjectsOfTypeAll<VFavoritesWindow>().Length > 0) return;
        ...
        public static MethodInfo mi_VFolders_DrawBigFolderIcon = t_VFolders?.GetMethod("DrawBigFolderIcon_forVFavorites", maxBindingFlags);
